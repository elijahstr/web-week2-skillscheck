...
const me = {
	...
	state: "Utah",
	age: 21,
	greeter: function(){ `Hello! My name is ${this.firstName} and I live in ${this.state}`}
}
...
// Code Here
const doubled = myNumbers.map(function(element){return element*=2;})
...
// Code Here
const filtered = myNumbers.filter(function(element){return element>100;})
...
// Code Here
const total = myNumbers.reduce(function(acc, curr){return acc+curr;})
...
// Code Here
const forTheLoveOfGeorge = notGeorge.map(function(element){return element = "George"});
...
for (let i = 0; i < people.length; i++) {
	people[i].friend ? null : enemies.push(people[i]);
}
...
for (let i = 0; i < people.length; i++) {
	temp.push(people[i].awesomeLevel);
}

const totallyAwesome = temp.reduce((acc, curr)=>acc+curr);
